...
using DataFrames
boxInd_df = DataFrame(n=Int[], N=Int[], Type=String[], mean=Union{Float64, Missing}[],
...
for n in [500, 1000, 2000, 4000]
  for k in 4:7
    ...
    println(n, " ", N)
    maxit = 10*n
    vs = CoupledConditionalSMC.couplingTimes(model, N, n, m, true, false, maxit)
    push!(boxInd_df, [n, N, "AT", mean(vs), std(vs)])
    vs = CoupledConditionalSMC.couplingTimes(model, lM, N, n, m, :AS, true, false, maxit)
    push!(boxInd_df, [n, N, "AS", mean(vs), std(vs)])
    vs = CoupledConditionalSMC.couplingTimes(model, lM, N, n, m, :BS, true, false, maxit)
    push!(boxInd_df, [n, N, "BS", mean(vs), std(vs)])
  end
end

using JLD2
@save "paper/boxInd.jld2" boxInd_df
