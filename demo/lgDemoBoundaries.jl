using RNGPool
using Plots
Plots.gr()

include("demo.jl")
include("lgModel.jl")

verbose = false

setRNGs(12345)
model, lM, _ = setupLGModel(1000, 1.0)
@time b11 = CCSMCBoundaries(model, lM, 2, 10000, verbose); plot(b11)
@time b12 = CCSMCBoundaries(model, lM, 4, 10000, verbose); plot!(b12)
@time b13 = CCSMCBoundaries(model, lM, 8, 10000, verbose); plot!(b13)
@time b14 = CCSMCBoundaries(model, lM, 16, 10000, verbose); plot!(b14)
@time b15 = CCSMCBoundaries(model, lM, 32, 10000, verbose); plot!(b15)
plot(b11, label="N=2")
plot!(b12, label="N=4")
plot!(b13, label="N=8")
plot!(b14, label="N=16")
plot!(b15, label="N=32")
xlabel!("iteration")
ylabel!("boundary")
title!("Boundary against iteration for obsVariance = 1.0")
savefig("output/lgDemoBoundaries1.pdf")

setRNGs(12345)
model, lM, _ = setupLGModel(1000, 100.0)
@time b21 = CCSMCBoundaries(model, lM, 2, 10000, verbose); plot(b21)
@time b22 = CCSMCBoundaries(model, lM, 4, 10000, verbose); plot!(b22)
@time b23 = CCSMCBoundaries(model, lM, 8, 10000, verbose); plot!(b23)
@time b24 = CCSMCBoundaries(model, lM, 16, 10000, verbose); plot!(b24)
@time b25 = CCSMCBoundaries(model, lM, 32, 10000, verbose); plot!(b25)
plot(b21, label="N=2")
plot!(b22, label="N=4")
plot!(b23, label="N=8")
plot!(b24, label="N=16")
plot!(b25, label="N=32")
xlabel!("iteration")
ylabel!("boundary")
title!("Boundary against iteration for obsVariance = 100.0")
savefig("output/lgDemoBoundaries2.pdf")

setRNGs(12345)
model, lM, _ = setupLGModel(1000, 1000.0)
@time b35 = CCSMCBoundaries(model, lM, 32, 500, verbose); plot(b35)
@time b36 = CCSMCBoundaries(model, lM, 64, 500, verbose); plot!(b36)
@time b37 = CCSMCBoundaries(model, lM, 128, 500, verbose); plot!(b37)
@time b38 = CCSMCBoundaries(model, lM, 256, 500, verbose); plot!(b38)
plot(b35, label="N=32")
plot!(b36, label="N=64")
plot!(b37, label="N=128")
plot!(b38, label="N=256")
xlabel!("iteration")
ylabel!("boundary")
title!("Boundary against iteration for obsVariance = 1000.0")
savefig("output/lgDemoBoundaries3.pdf")
