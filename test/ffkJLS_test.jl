...
function _getFreqsJLS(model::SMCModel, lM::F, ccsmcio::CCSMCIO, algorithm::Symbol,
  ...
  for i = 1:m
    CoupledConditionalSMC.ccXpfJLS!(model, lM, ccsmcio, algorithm)
...
function testccsmc()
  ...
  freqs1, freqs2 = _getFreqsJLS(model, lM, ccsmcio, :BS, nsamples, d)

  testapproxequal(freqs1, densities, 0.05, false)
  testapproxequal(freqs2, densities, 0.05, false)

  freqs1, freqs2 = _getFreqsJLS(model, error, ccsmcio, :AT, nsamples, d)

  testapproxequal(freqs1, densities, 0.05, false)
  testapproxequal(freqs2, densities, 0.05, false)

  freqs1, freqs2 = _getFreqsJLS(model, lM, ccsmcio, :AS, nsamples, d)
